...
const Store = function(mediator, collection) {
  ...
  this.create = this.collection.create.bind(this.collection);
  this.find = this.collection.find.bind(this.collection);
  this.findOne = this.collection.findOne.bind(this.collection);
  this.findOrCreate = this.collection.findOrCreate.bind(this.collection);
  this.update = this.collection.update.bind(this.collection);
  this.destroy = this.collection.destroy.bind(this.collection);
...
// class-level methods
Store.model = function(name, config) {
...
Store.init = function(conf, cb) {
  waterline.initialize(conf, function(err, ontology) {
    if (err) {
      return cb(err);
    }
...
Store.prototype.listen = function(topicPrefix) {
  ...
  self.topics.create = "wfm:" + topicPrefix + self.collectionName + ':create';
  ...
  self.topics.load = "wfm:" + topicPrefix + self.collectionName + ':read';
  ...
  self.topics.save = "wfm:" + topicPrefix + self.collectionName + ':update';
  ...
  self.topics.delete = "wfm:" + topicPrefix + self.collectionName + ':delete';
  ...
  self.topics.list = "wfm:" + topicPrefix + self.collectionName + ':list';
